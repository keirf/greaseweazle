...
def align_track(usb: USB.Unit, args) -> None:
    ...
    if len(track_list) == 0:
        raise error.Fatal("Align command requires at least one track "
                          "(e.g., c=40:h=0)")
    ...
    if len(track_list) > 1:
        ...
        for track_info in track_list[1:]:
            if track_info[0] != cyl:
                raise error.Fatal("All tracks must be on the same cylinder "
                                  "for alignment")
    ...
    if len(track_list) == 1:
        ...
        print(f"Aligning T{cyl} (alternating heads {','.join(heads)}), "
              f"reading {args.reads} times, revs={args.revs}")
